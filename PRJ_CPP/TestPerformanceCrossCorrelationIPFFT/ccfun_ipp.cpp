...
void PhaseCorrelation(Ipp8u* ref, Ipp8u* src, IppiSize roi, int &wshift, int &hshift)
{
	...
	status = ippiFFTFwd_RToPack_32f_C1R(ref_tmp, roi.height * sizeof(Ipp32f), ref_tmp, roi.height * sizeof(Ipp32f), pFFTSpec, ipp8uBuffer);

	ippiConvert_8u32f_C1R(src, roi.width * sizeof(Ipp8u), src_tmp, roi.width * sizeof(Ipp32f), roi);
	status = ippiFFTFwd_RToPack_32f_C1R(src_tmp, roi.height * sizeof(Ipp32f), src_tmp, roi.height * sizeof(Ipp32f), pFFTSpec, ipp8uBuffer);
